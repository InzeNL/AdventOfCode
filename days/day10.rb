class Day10
  ...
  def get_trail_heads(lines, coordinate)
    x = coordinate[0]
    y = coordinate[1]
  ...
  def part2()
    ...
    coordinates.each do |coordinate|
      sum += get_trail_head_ratings(lines, coordinate)
    end

    return sum
  end

  def get_trail_head_ratings(lines, coordinate)
    x = coordinate[0]
    y = coordinate[1]
    ...
    if (up == current_number + 1)
      coordinates += get_trail_head_ratings(lines, [up_x, up_y])
    end

    if (right == current_number + 1)
      coordinates += get_trail_head_ratings(lines, [right_x, right_y])
    end

    if (down == current_number + 1)
      coordinates += get_trail_head_ratings(lines, [down_x, down_y])
    end

    if (left == current_number + 1)
      coordinates += get_trail_head_ratings(lines, [left_x, left_y])
    end
